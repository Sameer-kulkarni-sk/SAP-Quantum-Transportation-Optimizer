...
class QUBOFormulation:
    ...
    def __init__(self,
        ...
        self.weights = weights or {
            ...
            'co2': 0.1,            # CO₂ weight (scaled)
            'assignment': 1000.0,  # Assignment constraint penalty
            'capacity': 1000.0,    # Capacity constraint penalty
            'deadline': 500.0,     # Deadline constraint penalty
            'available': 2000.0    # Availability constraint penalty
        }
    ...
    def _add_assignment_penalty(self, linear: Dict, quadratic: Dict):
        ...
        for i in range(len(self.shipments)):
            # Linear term: -2λ * Σ(j) x[i,j]
            for j in range(len(self.trucks)):
                var = f'x_{i}_{j}'
                linear[var] = linear.get(var, 0) - 2 * lambda_assign

            # Quadratic term: λ * Σ(j) Σ(k) x[i,j] * x[i,k]
            for j in range(len(self.trucks)):
                for k in range(len(self.trucks)):
                    ...
                    if j == k:
                        # Diagonal term
                        linear[var_j] = linear.get(var_j, 0) + lambda_assign
                    else:
                        # Off-diagonal term
                        key = (var_j, var_k) if var_j < var_k else (
                            var_k, var_j)
                        quadratic[key] = quadratic.get(key, 0) + lambda_assign
