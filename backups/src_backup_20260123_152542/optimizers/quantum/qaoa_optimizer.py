...
class QAOAOptimizer(BaseOptimizer):
    ...
    def optimize(self,
        ...
        try:
            ...
            print(
                f"QUBO problem size: {n_vars} variables, {n_constraints} constraints")

            # Check if problem is too large for quantum simulation
            if n_vars > 20:
                print(
                    f"Warning: Problem size ({n_vars} variables) may be too large for efficient quantum simulation")
                print("Consider using a smaller problem or classical optimizer")
    ...
    def optimize_with_fallback(self,
        ...
        if n_vars > 20:
            print(
                f"Problem too large for QAOA ({n_vars} variables). Using classical optimizer.")
        ...
        return self.optimize(weights=weights, **kwargs)
